...
class ProEncoder:
    ...
    # 利用str.maketrans()函数，将protein的输入表和输出表转换为字典
    # clusters: {A,G,V}, {I,L,F,P}, {Y,M,T,S}, {H,N,Q,W}, {R,K}, {D,E}, {C}
    pro_intab = 'AGVILFPYMTSHNQWRKDEC'  # 定义蛋白质输入表
    pro_outtab = 'AAAIIIIYYYYHHHHRRDDC'  # 定义蛋白质输出表

    def __init__(self, WINDOW_P_UPLIMIT, WINDOW_P_STRUCT_UPLIMIT, CODING_FREQUENCY,
        ...
        参数:
        - WINDOW_P_UPLIMIT: int，窗口的最大长度。
        - WINDOW_P_STRUCT_UPLIMIT: int，结构窗口的最大长度。
        - CODING_FREQUENCY: bool，是否进行频率编码。
        - TRUNCATION_LEN: int，截断长度，默认为None。
        - PERIOD_EXTENDED: bool，是否周期性扩展，默认为None。
        ...
        # 通过滑动窗口的方式生成所有可能的由self.elements元素(氨基酸按偶极矩和侧链体积分类为七种)组成的长度不断增加的子串序列（即k-mers）
        # 建立这些k-mers与其在全局k-mer列表中索引位置的对应关系。
        k_mers = ['']
        self.k_mer_list = []
        self.k_mer_map = {}
        ...
        # 创建了一个字符映射表（transtable），用于将字符串中的字符从self.pro_intab映射到对应的字符self.pro_outtab。
...
class RNAEncoder:
    ...
    def __init__(self, WINDOW_R_UPLIMIT, WINDOW_R_STRUCT_UPLIMIT, CODING_FREQUENCY,
                 TRUNCATION_LEN=None, PERIOD_EXTENDED=None):
    ...
    def encode_conjoint(self, seq):
        seq = seq.replace('T', 'U')  # 将字符串中的所有'T'替换为'U‘
        seq = ''.join([x for x in seq if x in self.elements])
        seq_len = len(seq)
        if seq_len == 0:
            return 'Error'
